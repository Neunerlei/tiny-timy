 ...
 */
class DateTimy extends DateTime
{
     ...
     */
    protected static $serverTimezone = "UTC";
    ...
    /**
     * DateTimy constructor.
     *
     * @param string|DateTimy|\DateTime|int $time Either a DateTime string, a unix timestamp or a formatted string
    ...
    public function __construct($time = "now", $timezone = NULL, ?string $format = NULL)
    {
        ...
        if (is_string($timezone)) {
            if ($timezone === "server") {
                ...
                if ($timezone === "client") {
                    $timezone = static::getClientTimeZone();
                }
            }
        }
        ...
        if (!empty($format) && !is_numeric($time)) {
            if (!empty(static::getFormat($format))) {
    ...
    public static function configureTimezone($timeZone, bool $server = FALSE): void
    {
        if (!$timeZone instanceof DateTimeZone) {
            if (is_string($timeZone)) {
                ...
                throw new InvalidArgumentException("The given timezone is invalid! Only strings and DateTimeZone objects are allowed!");
            }
        }
        ...
        if (!$server) {
            ...
            static::$serverTimezone = $timeZone;
        }
    }
    ...
    {
        if (is_string(static::$serverTimezone)) {
            return new DateTimeZone(static::$serverTimezone);
        }

        return static::$serverTimezone;
    }
    ...
    {
        if (empty(static::$clientTimezone)) {
    ...
    public function __call($name, $arguments)
    {
        ...
        if (static::$formatLocalization[$type]) {
